...
public class UserController {
    ...
    // key=값
    // 로그인 성공 시 -> welcome.mustache 파일로 이동 (토큰) - 리프레시 갱신 (폴링)
    ...
    // SSR 로 자동 갱신 로직을 만들어 본다면
    // 클라이언트가 401 Unauthorized 받으면 ajax로 refresh 요청하면 된다.
    @PostMapping("/refresh")
    public String refreshToken(@RequestParam  String refreshToken, Model model) {
        // 사용자가 던진 refreshToken 과 DB에 저장되어 있는 refreshToken 대조
        // 및 토큰 유효성 검사
