...
public class JWTUtil {
    ...
    // 사용자 로그인 --> username, password --> JWT 발급 (왕국 시크릿 키)
    // 액세스 토큰 생성(10분)
    public String generateAccessToken(String username) {
        try {
            ...
            return JWT.create()
                    ...
                    .withExpiresAt(new Date(System.currentTimeMillis() + 1000 * 60 * 10)) // 만료 시간을 10분으로 제한
                    .sign(algorithm);
    ...
    public String generateRefreshToken(String username) {
        try {
            ...
            return JWT.create()
                    ...
                    .withClaim("role", "USER")
                    .withIssuedAt(new Date()) // 페이로드에 발행시간 설정
                    .withExpiresAt(new Date(System.currentTimeMillis() + 1000 * 30)) // 만료 시간을 7일로 제한
                    .sign(algorithm);
        } catch (JWTCreationException e) {
    ...
    // 토큰 유효성 검사
    // 토큰 시간 유효한지, 위 변조 되지 않았는지 확인
    public boolean validateToken(String token, String username) {
        try {
            Algorithm algorithm = Algorithm.HMAC256(secret);
            JWT.require(algorithm) //  이 알고리즘으로 서명된 토큰만 허용
                    .build()
                    .verify(token); // 토큰 검증 (만료시간, 위조체크)
            String extractUsername = extractUsername(token);
            return extractUsername != null &&extractUsername.equals(username);
        } catch (Exception e) {
            return false;
        }
